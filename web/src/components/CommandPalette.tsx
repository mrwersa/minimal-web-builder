  ...
  return (
    ...
    >
      <section
        ...
        className="w-full max-w-lg overflow-hidden rounded-xl border border-border2 bg-surface shadow-2xl"
      >
        <div className="flex items-center gap-2 border-b border-border2 px-3">
          <Command className="h-4 w-4 shrink-0 text-muted2" />
          <Input
            autoFocus
            role="combobox"
            aria-expanded="true"
          ...
          />
          <kbd className="rounded border border-border2 bg-bg px-1.5 py-0.5 text-[10px] text-muted2">
            Esc
          </kbd>
        </div>
        <div id="builder-command-list" role="listbox" className="max-h-80 overflow-y-auto p-2">
          {filtered.length === 0 && (
            <p className="px-3 py-6 text-center text-xs text-muted2">No commands found.</p>
          )}
          {filtered.map((command, index) => (
            <button
              ...
              className={
                index === activeIndex
                  ? "flex w-full items-center gap-3 rounded-lg bg-accentSoft px-3 py-2 text-left text-accent"
                  : "flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-text2 hover:bg-bg disabled:opacity-40"
              }
            >
              ...
              {command.shortcut && (
                <kbd className="shrink-0 rounded border border-border2 bg-surface px-1.5 py-0.5 text-[10px] text-muted2">
                  {command.shortcut
